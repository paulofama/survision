...
-- Eso tiene dos defectos, confirmados con Paulo el 31/08/2026:
--
--   1. El costo del pool es ANUAL (las 14 unidades de fluoresceína del pool de
--      consultorio son de un año), pero se divide por cantidades MENSUALES.
--      Sobra un factor 12.
--   2. Las estimaciones no se parecen a la realidad. Exoftalmología figura con
--      9 prácticas por mes y hace 859.
--
-- Combinados, el sistema asigna 20,9 veces lo que los pools cuestan: en 8 meses
-- de 2026 reparte $36.753.232 sobre un costo real de $1.760.807. Un prorrateo
-- no puede crear costo — solo repartir el que hay.
--
-- CÓMO QUEDA
-- ----------
--   costo unitario = fn_costo_pool(pool)              [costo ANUAL del pool]
--                    / volumen ANUAL real del pool     [de movimientos_geclisa,
--                                                       últimos 12 meses]
--
-- El volumen sale de GECLISA, así que el prorrateo cierra solo y nadie tiene que
-- mantener 103 estimaciones al día.
--
-- EL PISO DE 12
-- -------------
-- Dividir por el volumen real rompe en los pools de poco uso: "Re Esterilizable
-- + Lavado" tuvo 2 prácticas en el año, y con eso su unitario da $53.238 — que
-- se duplicaría si el año próximo hubiera una sola, y quedaría indefinido si no
-- hubiera ninguna.
--
-- Por eso nunca se divide por menos de VOLUMEN_MINIMO (12). En esos pools el
-- prorrateo NO cierra: queda costo del pool sin asignar. Es deliberado —
-- preferimos dejar costo sin repartir antes que inflar el unitario de una
-- práctica que casi no se hace. La vista lo expone en `pool_bajo_piso` para que
-- la pantalla lo pueda advertir en vez de que pase inadvertido.
--
-- POR QUÉ ES UNA VISTA PARALELA
-- -----------------------------
-- El cambio baja el costo asignado de 2026 en unos $35 M y sube el margen de
-- contribución en la misma cifra, casi todo en Consultas. Es demasiado para
-- aplicarlo a ciegas: esta migración NO toca `v_recetas_costos_por_pool`, que
-- sigue siendo la que consume el módulo. Crea `v_recetas_costos_por_pool_v2`
-- para poder comparar los dos informes antes de decidir el cambio.
--
...
-- ------------------------------------------------------------
-- Volumen mínimo por el que se divide. Si un pool tuvo menos prácticas que
-- esto en los últimos 12 meses, se usa este número igual.
-- ------------------------------------------------------------
  ...
  SELECT p.id AS pool_id,
         ...
         CASE
           WHEN COALESCE(vp.volumen_usado, 0) > 0
             THEN fn_costo_pool(p.id) / vp.volumen_usado
           ELSE 0::numeric
