// ============================================================
// CARGADOR de F.931 2025 (Fase 3/4) - Modulo Sueldos
// ============================================================
// USO:
//   cd server
//   node scripts/cargar-f931-2025.js            -> DRY-RUN (parsea y muestra)
//   node scripts/cargar-f931-2025.js --write     -> sube PDF + crea declaracion confirmada
//
// Para cada mes 01..11 de 2025 (12 ya cargado por UI):
//   1. Lee el PDF 'C:\FISCAL\931\931\F 931 MM2025.pdf'.
//   2. Lo parsea con services/f931Parser (mismo parser de la UI).
//   3. Con --write: sube el PDF al bucket sueldos-adjuntos, inserta la
//      declaracion en estado REVISADO_CONFIRMADO (linkeada a la liquidacion)
//      y el adjunto. Replica crearDeclaracion(confirmarAlGuardar=true) del front.
//
...
(async () => {
  log('='.repeat(70));
  log(`CARGA F.931 ${ANIO} — ${WRITE ? 'MODO ESCRITURA' : 'DRY-RUN'}`);
  log('='.repeat(70));
  ...
  for (const mes of MESES) {
    ...
    log(`\n${mm}/${ANIO}  cuit_ok=${res.cuit_coincide}  periodo_ok=${!!periodoOk}  trab=${c.cantidad_trabajadores}` +
        (yaExiste ? `  [YA EXISTE: ${yaExiste.estado}]` : '') + (liq ? '' : '  [SIN LIQUIDACION]'));
    ...
    if (!liq) { log('   -> sin liquidacion, no se puede linkear; salteo.'); continue; }
  ...
  log('\n' + '='.repeat(70));
  if (!WRITE) log('DRY-RUN. Para escribir: node scripts/cargar-f931-2025.js --write');
