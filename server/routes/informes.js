...
router.get('/gestion-mensual', async (req, res) => {
  ...
  try {
    ...
    const evolOSResult = await executeQuery(queryEvolOS, {
      fechaIni12M,
      fechaFin12M,
    });
    ...
    const evolPrestadorResult = await executeQuery(queryEvolPrestador, {
      fechaIni12M,
      fechaFin12M,
    });
    ...
    const evolPracticaResult = await executeQuery(queryEvolPractica, {
      fechaIni12M,
      fechaFin12M,
    });

    // ============================================================
    // QUERY 19: CRUCE PRÁCTICAS × OBRAS SOCIALES (mes actual)
    // ============================================================
    // Devuelve para el mes actual: cantidad y facturado por cada
    // combinación práctica-OS. El frontend arma el ranking de top 10
    // OS y agrupa el resto en "Otras".
    const queryCruceOSPracticas = `
      ...
      WHERE me.Me_Fecha BETWEEN @fechaIniMesActual AND @fechaFinMesActual
      GROUP BY n.nom_id, n.nom_cod, n.nom_nom, os.os_id, os.os_sigla, os.os_nombre
    `;

    const cruceOSPracticasResult = await executeQuery(queryCruceOSPracticas, {
      fechaIniMesActual,
      fechaFinMesActual,
    });
    ...
    console.log(`📈 [INFORMES] Evolución 12M: ${evolucion12Meses.obrasSociales.length} OS, ${evolucion12Meses.prestadores.length} prestadores, ${evolucion12Meses.practicas.length} prácticas`);

    // ============================================================
    // PROCESAMIENTO CRUCE OS × PRÁCTICAS (mes actual)
    // ============================================================
    // Construye matriz: filas = todas las prácticas, columnas = top 10 OS + "Otras".
    // Cada celda contiene { cantidad, facturado }.
    const filasCruce = cruceOSPracticasResult.recordset;

    // 1) Ranking de OS por facturación TOTAL del mes (cualquier práctica)
    const osTotales = new Map();
    filasCruce.forEach(r => {
      const osId = r.osId;
      if (osId === null || osId === undefined) return; // ignorar atenciones sin OS
      let entry = osTotales.get(osId);
      if (!entry) {
        entry = {
          osId,
          osSigla: (r.osSigla || '').trim(),
          osNombre: (r.osNombre || '').trim(),
          totalFacturado: 0,
        };
        osTotales.set(osId, entry);
      }
      entry.totalFacturado += parseFloat(r.facturado || 0);
    });
    const rankingOS = Array.from(osTotales.values())
      .sort((a, b) => b.totalFacturado - a.totalFacturado);
    const top10OS = rankingOS.slice(0, 10);
    const top10Set = new Set(top10OS.map(o => o.osId));

    // 2) Construir matriz por práctica
    ...
    filasCruce.forEach(r => {
      ...
      if (!prac) {
        prac = {
          nomId: r.nomId,
          nomCod: (r.nomCod || '').trim(),
          nomNombre: (r.nomNombre || '').trim(),
          totalCantidad: 0,
          totalFacturado: 0,
          celdas: {},   // { osId | 'OTRAS': { cantidad, facturado } }
        };
        practicasMap.set(key, prac);
      }
      const cant = parseInt(r.cantidad || 0);
      ...
      // ¿Va al top 10 o a "Otras"?
      const colKey = (r.osId !== null && r.osId !== undefined && top10Set.has(r.osId))
        ? String(r.osId)
        : 'OTRAS';
      let celda = prac.celdas[colKey];
      if (!celda) {
        celda = { cantidad: 0, facturado: 0 };
        prac.celdas[colKey] = celda;
      }
      celda.cantidad += cant;
      celda.facturado += fact;
    });

    // 3) Ordenar prácticas por facturación total DESC
    const cruceOSxPracticas = {
      columnasOS: top10OS.map(o => ({
        osId: o.osId,
        sigla: o.osSigla || o.osNombre.substring(0, 10) || 'S/D',
        nombre: o.osNombre,
      })),
      filasPracticas: Array.from(practicasMap.values())
        .filter(p => p.totalCantidad > 0)
        .sort((a, b) => b.totalFacturado - a.totalFacturado),
    };

    console.log(`🔀 [INFORMES] Cruce OS×Prácticas: ${cruceOSxPracticas.filasPracticas.length} prácticas × ${cruceOSxPracticas.columnasOS.length} OS top + Otras`);
    ...
    const response = {
      ...
      // ✅ Evolución mensual últimos 12 meses (cantidades, sin importes)
      evolucion12Meses,

      // ✅ Cruce práctica × OS (mes actual) — todas las prácticas, top 10 OS + Otras
      cruceOSxPracticas,
    };
