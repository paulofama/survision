...
const { generarComparativa } = require('../services/analisisExtractor');
...
router.get('/comparativa-inteligente', async (req, res) => {
  try {
    // El cómputo vive en analisisExtractor.js (misma fuente que usa el daemon
    // para el snapshot de Supabase). Así no hay drift entre LAN y remoto.
    const data = await generarComparativa();
    console.log(`✅ Comparativa inteligente calculada - Día ${data.periodo.diaActual}/${data.periodo.diasEnMes}`);
    res.json(data);
  } catch (error) {
